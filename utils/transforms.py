...
from utils.img_utils import normalize_ct, segment_ct
from utils.img_utils import crop_image, load_as_array
...
class AddChannel:
    ...
    def __init__(self, img_add: bool = True, msk_add: bool = True):
        self.flag = [img_add, msk_add]

    def __call__(self, image, mask=None):
        if self.flag[0]:
            image = image[None]
        if self.flag[1] and mask is not None:
            mask = mask[None]
        return image, mask


class CropForeground:
    ''' crop foreground of CT image '''

    def __init__(self, threshold: int = -420):
        self.threshold = threshold

    def __call__(self, image: np.ndarray, mask: np.ndarray = None):
        image = segment_ct(image, self.threshold)
        return image, mask
