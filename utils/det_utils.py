...
#         ebox_origin = np.load(os.path.join(
#             prep_dir, name + '_ebox_origin.npy'))
...
#             submission.append(
#                 [name, worldCoord[2], worldCoord[1], worldCoord[0], p[0]])
...
#         labels.flatten(), y_pred.flatten(), bins=(true_objects, pred_objects))[0]
...
def ext2factor(bboxes, factor=8):
    ...
    bboxes[:, 3:] = bboxes[:, 3:] // factor * factor + \
        (bboxes[:, 3:] % factor != 0).astype(np.int32) * factor
...
#         D_c, H_c, W_c = m.shape
#         zoomed_crop = zoom(m,
#                            (float(z_end - z_start) / D_c, float(y_end -
#                             y_start) / H_c, float(x_end - x_start) / W_c),
#                            order=2)
#         mask[cat - 1][z_start:z_end, y_start:y_end,
#                       x_start:x_end] = (zoomed_crop > 0.5).astype(np.uint8)
...
#         D_c, H_c, W_c = m.shape
#         mask[cat - 1][z_start:z_end, y_start:y_end,
#                       x_start:x_end] = (m > 0.5).astype(np.uint8)
...
#     # D, H, W = masks.shape
#     instance_nums = [num for num in np.unique(
#         masks) if num]  # 按照预处理设置的的编号获取肺结节
...
#         #         print coord_boxes[i]
#         #         print masks[i].shape
#         mask[cat][z_start:z_end, y_start:y_end,
#                   x_start:x_end] = masks[i].astype(np.uint8)
...
#         pred = y_pred == i
#         if target.sum():
#             score = 2 * (target * pred).sum() / \
#                 float((target.sum() + pred.sum()))
...
# def hausdorff_distance(y_pred, y_true, spacing=[1., 1., 1.], percent=0.95, num_class=len(config['roi_names']),
#                        decimal=4):
#     """
