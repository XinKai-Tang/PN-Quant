...
from .layer import *
from .net_configs import net_cfgs as DEFAULT_CFGS
from utils.det_utils import center_box_to_coord_box, clip_boxes, ext2factor
...
class RpnHead(nn.Module):
    def __init__(self,
        ...
        self.conv = nn.Sequential(nn.Conv3d(in_channels, 64, kernel_size=1),
                                  nn.ReLU())
        self.logits = nn.Conv3d(64, 1 * self.n_anchors, kernel_size=1)
        self.deltas = nn.Conv3d(64, 6 * self.n_anchors, kernel_size=1)
...
class MaskHead(nn.Module):
    ...
    def forward(self, detections, features):
        ...
        for detection in detections:
            ...
            up1 = f_4[b, :, z_start / 4:z_end / 4, y_start /
                      4:y_end / 4, x_start / 4:x_end / 4].unsqueeze(0)
            up2 = self.up2(up1)
            up2 = self.back2(torch.cat((up2, f_2[b, :, z_start / 2:z_end / 2, y_start / 2:y_end / 2, 
                                                 x_start / 2:x_end / 2].unsqueeze(0)), 1))
            up3 = self.up3(up2)
            im = img[b, :, z_start:z_end, y_start:y_end,
                     x_start:x_end].unsqueeze(0)
...
class CropRoi(nn.Module):
    def __init__(self, rcnn_crop_size, stride, crop_size):
        super(CropRoi, self).__init__()
        self.rcnn_crop_size = rcnn_crop_size
        self.scale = stride
        self.DEPTH, self.HEIGHT, self.WIDTH = crop_size
...
class NoduleNet(nn.Module):
    ''' NoduleNet for Lung Nodules Detection '''
    mode = None

    def __init__(self, cfg: dict = DEFAULT_CFGS, mode: str = 'test'):
        super(NoduleNet, self).__init__()

        self.cfg = cfg
        self.set_mode(mode)
        self.feature_net = FeatureNet(1, 128)
        self.rpn = RpnHead(in_channels=128,
                           n_anchors=len(self.cfg["anchors"]))
        self.rcnn_head = RcnnHead(in_channels=64,
                                  n_classes=self.cfg["num_class"],
                                  crop_size=self.cfg["rcnn_crop_size"])
        self.rcnn_crop = CropRoi(rcnn_crop_size=cfg['rcnn_crop_size'],
                                 stride=self.cfg["stride"],
                                 crop_size=self.cfg["crop_size"])
        self.mask_head = MaskHead(in_channels=128,
                                  n_classes=self.cfg["num_class"])
        self.use_rcnn = False
        self.use_mask = False
    ...
    def forward(self, inputs, truth_boxes, truth_labels, truth_masks, masks, split_combiner=None, nzhw=None):
        ...
        if self.mode in ['train', 'valid']:
            ...
            self.rpn_labels, self.rpn_label_assigns, self.rpn_label_weights, \
            self.rpn_targets, self.rpn_target_weights = make_rpn_target(
                self.cfg, self.mode, inputs, self.rpn_window, truth_boxes, truth_labels
            )
        ...
        if self.use_rcnn:
            ...
            if self.use_mask and len(self.detections):
                ...
                features = [t.unsqueeze(0).expand(torch.cuda.device_count(), -1, -1, -1, -1, -1)
                            for t in features]
                self.mask_probs = data_parallel(
                    self.mask_head, (torch.from_numpy(self.crop_boxes).cuda(), features))

                if self.mode in ['eval', 'test']:
                    mask_keep = mask_nms(
                        self.cfg, self.mode, self.mask_probs, self.crop_boxes, inputs)
    ...
    def forward2(self, inputs, bboxes):
        ...
        for b in range(len(bboxes)):
            self.crop_boxes.append(np.column_stack((
                np.zeros((len(bboxes[b]) + b, 1)), bboxes[b]
            )))
    ...
    def forward_mask(self, inputs, truth_boxes, truth_labels, truth_masks, masks, split_combiner=None, nzhw=None):
        ...
        for b in range(len(truth_boxes)):
            self.crop_boxes.append(np.column_stack((
                np.zeros((len(truth_boxes[b]) + b, 1)), truth_boxes[b], truth_labels[b]
            )))
        self.crop_boxes = np.concatenate(self.crop_boxes, 0)
        self.crop_boxes[:, 1:-1] = center_box_to_coord_box(self.crop_boxes[:, 1:-1])
        self.crop_boxes = self.crop_boxes.astype(np.int32)
    ...
    def loss(self, targets=None):
        ...
        self.rcnn_cls_loss, self.rcnn_reg_loss = torch.zeros(
        ...
        self.rpn_cls_loss, self.rpn_reg_loss, rpn_stats = \
            rpn_loss(self.rpn_logits_flat, self.rpn_deltas_flat, self.rpn_labels, self.rpn_label_weights,
                     self.rpn_targets, self.rpn_target_weights, self.cfg, mode=self.mode)
        ...
        self.total_loss = self.rpn_cls_loss + self.rpn_reg_loss \
            + self.rcnn_cls_loss + self.rcnn_reg_loss + self.mask_loss
