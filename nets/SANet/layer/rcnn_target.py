...
from .rcnn_nms import rcnn_encode
from torch.autograd import Variable

from utils.det_utils import py_box_overlap as torch_overlap
...
def make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label):
    ...
    if len(truth_box) == 0:
        ...
        bg_index = bg_index[
            np.random.choice(bg_length, size=num_bg,
    ...
    num_fg = int(np.round(cfg['rcnn_train_fg_fraction']
    ...
    if fg_length > 0 and bg_length > 0:
        ...
        bg_index = np.random.choice(
            num_proposal, size=num_bg, replace=num_proposal < num_bg)
    ...
    # Clamp label assignments for the background to -1
    sampled_assign[num_fg:] = -1
    ...
    if num_fg > 0:
        ...
        sampled_target = rcnn_encode(
            target_box, target_truth_box, cfg['box_reg_weight'])
...
def make_rcnn_target(cfg, mode, inputs, proposals, truth_boxes, truth_labels):
    ...
    for b in range(batch_size):
        ...
        proposal = add_truth_box_to_proposal(
            cfg, proposal, b, truth_box, truth_label)
