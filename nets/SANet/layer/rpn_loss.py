...
def binary_cross_entropy_with_hard_negative_mining(logits, labels, weights, batch_size, num_hard=2):
    ...
    if num_hard > 0:
        neg_prob, neg_labels = OHEM(
            neg_prob, neg_labels, num_hard * len(pos_prob))
...
def weighted_focal_loss_with_logits(logits, labels, weights, gamma=2.):
    ...
    loss = ((pos_loss * pos_weights).sum() + (neg_loss *
            neg_weights).sum()) / (weights.sum() + 1e-12)
...
def rpn_loss(logits, deltas, labels, label_weights, targets, target_weights, cfg, mode='train', delta_sigma=3.0):
    ...
    for i in range(6):
        ...
        rpn_reg_loss += l
        reg_losses.append(l.cpu().data.item())

    return rpn_cls_loss, rpn_reg_loss, [pos_correct.cpu(), pos_total,
                                        neg_correct.cpu(), neg_total,
