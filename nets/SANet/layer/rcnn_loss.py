...
def rcnn_loss(logits, deltas, labels, targets, deltas_sigma=1.0):
    ...
    rcnn_cls_loss = F.cross_entropy(
        logits, labels, weight=weight, size_average=True)
    ...
    for i in labels.nonzero():
        i = i.item()
        confusion_matrix[labels.long().cpu().detach()[i].item()
                         ][cat[i].cpu().detach().item()] += 1
    ...
    if num_pos > 0:
        ...
        select[:, 0] = 0
        select = select.view(batch_size, num_class, 1).expand(
            (batch_size, num_class, 6)).contiguous().byte()
        select = select.bool()
