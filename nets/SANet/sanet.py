...
class RpnHead(nn.Module):
    def __init__(self, config, in_channels=128):
        ...
        self.n_anchors = len(config['anchors'])
    ...
    def forward(self, f):
        ...
        logits = logits.view(size[0], size[2], size[3], size[4], self.n_anchors, 1)
        
        size = deltas.size()
        deltas = deltas.view(deltas.size(0), deltas.size(1), -1)
        deltas = deltas.transpose(1, 2).contiguous()
        deltas = deltas.view(size[0], size[2], size[3], size[4], self.n_anchors, 6)
    ...
    def forward(self, detections, features):
        ...
        for detection in detections:
            ...
            up2 = self.back2(torch.cat(
                (up2, f_2[b, :, z_start//2:z_end//2, y_start//2:y_end//2, x_start//2:x_end//2].unsqueeze(0)),
                1))
            up3 = self.up3(up2)
...
class CropRoi(nn.Module):
    ...
    def forward(self, f, inputs, proposals):
        ...
        for p in proposals:
            ...
            c0 = np.array([z_start.cpu(), y_start.cpu(), x_start.cpu()])
            c1 = np.array([z_end.cpu(), y_end.cpu(), x_end.cpu()])
            ...
            fe2 = self.back2(torch.cat(
                (fe1_up, out1[b, :, z_start//2:z_end//2, y_start//2:y_end//2, x_start//2:x_end//2].unsqueeze(0)),
                1))
            # fe2_up = self.up3(fe2)
...
class SANet(nn.Module):
    def __init__(self, cfg: dict = DEFAULT_CFGS, mode: str = 'train'):
        super(SANet, self).__init__()

        self.cfg = cfg
        self.set_mode(mode)
        self.feature_net = FeatureNet(self.cfg)
        self.rpn = RpnHead(self.cfg, in_channels=128)
        self.rcnn_head = RcnnHead(self.cfg, in_channels=64)
